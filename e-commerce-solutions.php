<!DOCTYPE html>
<html dir="ltr" lang="zxx">

<head>
    <meta name="viewport" content="width=device-width,initial-scale=1.0" />
    <meta charset="utf-8" />
    <meta name="description" content="Auctech Marketing Pvt. Ltd." />
    <meta name="keywords" content="AuctechMarketingPvt.Ltd." />
    <meta name="author" content="Auctech Marketing Pvt. Ltd." />

    <title>Auctech Marketing Pvt. Ltd. | E-commerce Solutions</title>

    <link rel="icon" type="image/png" href="images/logo/favicon.png" />

    <link rel="stylesheet" href="css/font-main.css">
    <link rel="preconnect" href="https://fonts.googleapis.com/">
    <link rel="preconnect" href="https://fonts.gstatic.com/" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Rubik:ital,wght@0,300..900;1,300..900&amp;display=swap"
        rel="stylesheet">
    <link rel="stylesheet" href="css/bootstrap.min.css" />
    <link rel="stylesheet" href="css/fontawesome.min.css">
    <link rel="stylesheet" href="css/magnific-popup.min.css" />
    <link rel="stylesheet" href="css/slick.min.css" />
    <link rel="stylesheet" href="css/nice-select.min.css" />
    <link rel="stylesheet" href="css/animate.min.css" />
    <link rel="stylesheet" href="css/style.css" />
    <script src="js/jquery.min.js"></script>
    <style>
     .service-details-page .service-details-wcu-title {
            height: 328px;
            
        }
        .pricing-area-card-wrapper {
            padding: 37px 25px 37px 25px;
        }
        .service-details-page .wcu-area .wuc-area-side-img {
            margin-top: 0px;
        }
        .mt-3{
            text-align:left !important;
        }
    </style>

</head>

<body>
    <!-- Preloader Start  -->
    <div id="preloader">
        <div id="preloader-status">
            <div id="wifi-loader">
                <svg viewBox="0 0 86 86" class="circle-outer">
                    <circle r="40" cy="43" cx="43" class="back"></circle>
                    <circle r="40" cy="43" cx="43" class="front"></circle>
                    <circle r="40" cy="43" cx="43" class="new"></circle>
                </svg>
                <svg viewBox="0 0 60 60" class="circle-middle">
                    <circle r="27" cy="30" cx="30" class="back"></circle>
                    <circle r="27" cy="30" cx="30" class="front"></circle>
                </svg>
                <div data-text="Loading..." class="text"></div>
            </div>
        </div>
    </div>
    <!-- Preloader End  -->
    <!-- Header Start -->
    <header id="header">
        <?php
        include('header.php');
        ?>
    </header>
    <!-- Header End -->
    <!-- Off Canvas Start  -->
    <div class="off-canvas-menu">
        <div class="off-canvas-menu-wrapper" style="background: #333333;">
            <div class="off-canvas-menu-header d-flex justify-content-between mb-40">
                <div class="logo">
                    <img src="images/logo/logo.png" alt="">
                </div>
                <div class="close-icon">
                    <i class="fa-solid fa-xmark"></i>
                </div>
            </div>
            <div class="content">
                <h4 class="mb-30 text-center">Get Appointment</h4>
                <div class="appatment-form">
                    <form action="#" method="post">
                        <input type="text" placeholder="Name" id="name" name="name" required>

                        <input type="email" id="email" name="email" placeholder="Email Address" required>

                        <textarea id="message" name="message" placeholder="Message" required></textarea>

                        <div class="appertment-btn">
                            <a href="#">Submit Now</a>
                        </div>
                    </form>
                </div>
                <div class="team-area-member-social-link offcanves-socail-icon  text-center mt-35">
                    <ul>
                        <li class="d-inline"><a href="#"><i class="fa-brands fa-facebook-f"></i></a></li>
                        <li class="d-inline"><a href="#"><i class="fa-brands fa-twitter"></i></a></li>
                        <li class="d-inline"><a href="#"><i class="fa-brands fa-linkedin-in"></i></a></li>
                        <li class="d-inline"><a href="#"><i class="fa-solid fa-basketball"></i></a></li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
    <div class="off-canvas-menu-overlay">
    </div>
    <!-- Off Canvas End  -->
    <!-- Services Details Page Start -->
    <div class="service-details-page  body-black ">
        <div class="pages-heading p-relative">
            <div class="pages-heading-image p-relative z-index-1">
               <img src="images/contact/contact-header-img.png" alt="">
            </div>
            <div class="pages-heading-text style-2 p-absolute z-index-1">
                <span>Home / E-commerce Solutions</span>
                <h2>E-commerce Solutions</h2>
            </div>
        </div>
        <!-- Service Details Wcu One Start -->
        <div class="service-details-wcu pt-140 p-relative pb-140">
            <div class="container">
                <div class="row">
                    <div class="col-xl-7 col-lg-7">
                        <div class="service-details-wcu-wrapper">
                            <div class="service-details-wcu-content">
                                <div class="service-details-wuc-img tilt-zoom">
                                    <img src="images\services\e-commerce\Smart Solutions for Modern Online Retail.jpg" alt="">
                                </div>
                            </div>
                            <div class="service-details-wcu-title"
                                style="background-image: url(images/shape/service-details-pag-title-bg.png);">

                                <h3>Smart Solutions for Modern <span>Online Retail</span></h3>
                            </div>
                        </div>
                    </div>
                    <div class="col-xl-5 col-lg-5">
                        <div class="section-title mb-32 style-1">
                            <h2>E-commerce Solutions</h2>
                        </div>
                        <div class="wcu-area-card-info-secondary">
                            <div class="wcu-area-info-card-wrapper">

                                <p>Empower your business with our end-to-end e-commerce solutions that simplify online
                                    selling, enhance customer experience, and boost your revenue. Whether you’re a small
                                    business, a growing brand, or an established enterprise, our expert team will design
                                    and implement a seamless e-commerce platform tailored to your needs. From store
                                    creation to payment integration and inventory management, we cover it all.</p>
                                
                            </div>
                            <div class="wcu-area-info-card-wrapper">
                                <span class="mb-15 fs-5 fw-bold">Our Key Services</span>
                                <p class="mt-3">
                                    <img width="20" height="20"
                                        src="https://img.icons8.com/glyph-neue/64/FAB005/instagram-verification-badge.png" />
                                    <strong class="text-white">Store Setup: </strong>End-to-end development of your
                                    online shop.
                                </p>
                                <p class="mt-3">
                                    <img width="20" height="20"
                                        src="https://img.icons8.com/glyph-neue/64/FAB005/instagram-verification-badge.png" />
                                    <strong class="text-white"> Payment Gateway Integration: </strong>Secure, seamless payments.
                                </p>
                                <p class="mt-3">
                                    <img width="20" height="20"
                                        src="https://img.icons8.com/glyph-neue/64/FAB005/instagram-verification-badge.png" />
                                    <strong class="text-white">Inventory Management: </strong> Real-time stock tracking and automation

                                </p>

                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- Pricing Page Star -->
        <div class="pricing-page body-black ">
            <!-- Pricing Area Start  -->
            <div class="pricing-area body-black pb-140 ">
                <div class="container">
                    <div class="section-title mb-60 style-2">

                        <h2>Our Services</h2>
                    </div>
                    <div class="row">
                        <div class="col-xl-4 col-lg-4">
                            <div class="pricing-area-card p-relative" id="cards-1">
                                <div class="pricing-area-card-wrapper">
                                    <div class="pricing-area-card-text">
                                        <h5>Store Setup</h5>

                                    </div>
                                    <div class="pricing-area-cards-list">
                                        <ul>
                                            <li class="checkmark">User-friendly, responsive online stores.
                                            </li>
                                            <li class="checkmark">Custom designs to match your brand.
                                            </li>
                                            <li class="checkmark">Mobile-optimized for easy access.
                                            </li>
                                            <li class="checkmark">Secure hosting & domain management.</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-4 col-lg-4">
                            <div class="pricing-area-card p-relative" id="cards-2">
                                <div class="pricing-area-card-wrapper">

                                    <div class="pricing-area-card-text">
                                        <h5>Inventory Management</h5>

                                    </div>
                                    <div class="pricing-area-cards-list">
                                        <ul>
                                            <li class="checkmark">Real-time stock tracking.</li>
                                            <li class="checkmark">Automated inventory updates.
                                            </li>
                                            <li class="checkmark">Manage categories & product variants.</li>
                                            <li class="checkmark">Stock performance reports.</li>
                                        </ul>

                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-4 col-lg-4 ">
                            <div class="pricing-area-card p-relative" id="cards-3">
                                <div class="pricing-area-card-wrapper">

                                    <div class="pricing-area-card-text">
                                        <h5>Payment Gateway Integration</h5>

                                    </div>
                                    <div class="pricing-area-cards-list">
                                        <ul>
                                            <li class="checkmark">Secure, reliable payment solutions.</li>
                                            <li class="checkmark"> Support for cards, wallets, & UPI.</li>
                                            <li class="checkmark"> Payment security compliance.
                                            </li>
                                            <li class="checkmark"> Automated confirmations & refunds.
                                            </li>
                                        </ul>

                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>

        <!-- Why Choose Us Start   -->
        <div class="wcu-area body-black p-relative pb-140 ">
            <div class="container">
                <div class="row ">
                    <div class="col-xl-8">
                    <div class="section-title mb-60 style-1 mb-4">
                    <h2>Showcase of multi-vendor platforms and scalable solutions</h2>
                </div>
                        <div class="wcu-area-card-info">
                            <div class="wcu-area-info-card-wrapper">
                                <h5> Multi-Vendor Platforms</h5>
                                <div class="title-border-3 mt-17 mb-30"></div>

                                <p>
                                    Creating a multi-vendor e-commerce platform offers opportunities for businesses to
                                    grow by collaborating with multiple sellers. At Auctech, we specialize in developing
                                    feature-rich platforms that provide vendors and customers with an exceptional
                                    experience.
                                </p>
                            </div>
                            <div class="wcu-area-info-card-wrapper">
                                <h5>Scalable Solutions</h5>
                                <div class="title-border-3 mt-17 mb-30"></div>
                                <p>
                                    As your business grows, your e-commerce platform must remain responsive and
                                    efficient. Our scalable solutions ensure your platform performs seamlessly, even
                                    with an increasing number of products, vendors, and customers.
                                </p>

                            </div>

                        </div>
                    </div>
                    <div class="col-xl-4 col-lg-4 col-md-4">
                        <div class=" wuc-area-side-img about-area-side-img p-relative">
                            <img src=" images\services\e-commerce\Multi-Vendor Platforms.png" alt="">
                            
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- Why Choose Us End   -->

    </div>
    <!-- Services Details Page End -->

    <!-- Footer Start -->
    <footer id="footer">
        <?php
        include('footer.php');
        ?>
    </footer>
    <!-- Scroll Up Section Start -->
    <div id="scrollTop" class="scrollup-wrapper">
        <div class="scrollup-btn">
            <i class="fa-solid fa-chevron-up"></i>
        </div>
    </div>
    <!-- Scroll Up Section End -->
    <script src="js/bootstrap.min.js"></script>
    <script src="js/jquery.nice-select.min.js"></script>
    <script src="js/slick.min.js"></script>
    <script src="js/jquery.counterup.min.js"></script>
    <script src="js/waypoints.js"></script>
    <script src="js/jquery.meanmenu.min.js"></script>
    <script src="js/jquery.magnific-popup.min.js"></script>
    <script src="js/inview.min.js"></script>
    <script src="js/wow.js"></script>
    <script src="js/tilt.jquery.js"></script>
    <script src="js/isotope.min.js"></script>
    <script src="js/jquery.imagesloaded.min.js"></script>
    <script src="js/custom.js"></script>

</body>

</html>